...
bundle from one immutable official Fiji archive and a checksummed overlay for
the declared PyImageJ, ImgLyb, and scyjava bridge versions. The overlay contains
the two ImgLib2 bridge JARs absent from the standard Fiji bundle together with
their license and source records. On first use, PolyStore downloads and
atomically materializes that complete runtime in the operating system's user
...
policy makes the materialized bundle the sole startup classpath, validates both
the active Java feature version and the ImageJ distribution, and does not
resolve pending scyjava endpoints. Process owners must close the gateway
through its ``shutdown`` method so the JVM is destroyed while the Python
runtime is still valid, rather than during interpreter finalization.
