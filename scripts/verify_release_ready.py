#!/usr/bin/env python3
"""
Verify that PolyStore is ready for PyPI release.
...
def check_version():
    ...
    init_file = Path("src/polystore/__init__.py")
    if not init_file.exists():
        print("  ❌ src/polystore/__init__.py not found")
        return False
    
    content = init_file.read_text()
    match = re.search(r'__version__\s*=\s*["\']([^"\']+)["\']', content)
    if not match:
        print("  ❌ __version__ not found in src/polystore/__init__.py")
        return False
...
def try_build():
    ...
    try:
        # Clean old builds
        import shutil
        for dir_name in ['dist', 'build', 'polystore.egg-info']:
...
def main():
    """Run all checks."""
    print("=" * 60, flush=True)
    print("PolyStore PyPI Release Readiness Check", flush=True)
    print("=" * 60, flush=True)
